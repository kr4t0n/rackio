...
export function mapItems(
  ...
  return metadata.slice(0, limit).map((entry, index) => {
    const progress =
      entry.duration && entry.viewOffset
    ...
    return {
      ...
      ...(entry.grandparentTitle ? { showTitle: entry.grandparentTitle } : {}),
      detail: describeItem(entry),
      progress,
      ...(artPath ? { artPath } : {}),
...
export async function fetchPlex(connection: PlexConnection): Promise<PlexState> {
  ...
  try {
    const [rootResponse, deckResponse] = await Promise.all([
    ...
    return {
      ...
      items: mapItems(
        ...
        connection.baseUrl,
        root.MediaContainer.machineIdentifier,
      ),
    };
  } catch (error) {
